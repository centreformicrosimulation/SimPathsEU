    ...
    public InSchoolAlignment(Set<Person> persons) {
        ...
        numEligible = persons.stream()
                .filter(person -> person.getLes_c4() != null
                        && person.getDag() >= MIN_STUDENT_AGE
                        && person.getDag() <= MAX_STUDENT_AGE)
                .count();
        ...
        numDeterministicStudents = persons.stream()
                .filter(person -> person.getLes_c4() != null
                        && person.getDag() >= MIN_STUDENT_AGE
                        && person.getDag() <= MAX_STUDENT_AGE
                        && Les_c4.Student.equals(person.getLes_c4_lag1())
                        && person.getDag() < Parameters.MIN_AGE_TO_LEAVE_EDUCATION)
                .count();
    ...
    public double evaluate(double[] args) {
        ...
        double expectedContinuing = persons.parallelStream()
                .filter(person -> person.getLes_c4() != null
                        ...
                        && Les_c4.Student.equals(person.getLes_c4_lag1())
                        && person.getDag() >= Parameters.MIN_AGE_TO_LEAVE_EDUCATION
                        && person.getDag() <= Parameters.MAX_AGE_TO_STAY_IN_CONTINUOUS_EDUCATION)
                .mapToDouble(person -> {
