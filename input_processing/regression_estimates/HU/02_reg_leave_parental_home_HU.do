...
* OBJECT: 			Final Probit and Linear Regression Models 
...
* Set Excel file 
* Info sheet
putexcel set "$dir_work/reg_leave_parental_home_HU", sheet("Info") replace
...
putexcel set "$dir_work/reg_leave_parental_home_HU", sheet("Gof") modify
...
* Need to add not partnered condition as well to be consistent with the simulation. 
			
fre dlftphm if (ded == 0 & dag >= 18) 
			
probit dlftphm i.dgn dag dagsq ib1.deh_c3 li.les_c3 li.ydses_c5 i.drgn1 stm ///
	y2020 y2021 if (ded == 0 & dag >= 18 & dcpst != 1) [pweight = dimxwt], ///
	vce(robust)
...
putexcel set "$dir_results/leave_parental_home/var_cov", sheet("var_cov") ///
	replace
putexcel A1 = matrix(V)

import excel "$dir_results/leave_parental_home/var_cov", sheet("var_cov") ///
	clear
...
mkmat v*, matrix(var)	
putexcel set "$dir_work/reg_leave_parental_home_HU", sheet("P1a") modify
...
Ydses_c5_Q5_L1 HUA HUB Year_transformed Y2020 Y2021 Constant
