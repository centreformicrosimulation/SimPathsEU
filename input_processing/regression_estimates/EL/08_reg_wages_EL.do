...
import excel "$dir_external_data/${country}_wages.xlsx", sheet("Wages") ///
	firstrow clear 
...
gen CPI = .
replace CPI = 99.27  if stm == 10
replace CPI = 102.36 if stm == 11
replace CPI = 103.42 if stm == 12
replace CPI = 102.54 if stm == 13
replace CPI = 101.11 if stm == 14
replace CPI = 100    if stm == 15
replace CPI = 100.02 if stm == 16
replace CPI = 101.15 if stm == 17
replace CPI = 101.94 if stm == 18
replace CPI = 102.46 if stm == 19
replace CPI = 101.17 if stm == 20
replace CPI = 101.75 if stm == 21
replace CPI = 111.21 if stm == 22
replace CPI = 115.84 if stm == 23
...
* Trim the sample 
sum wage_hour, det
replace wage_hour = . if wage_hour <= r(p1)
...
// Need to be observed at least once in the past 
...
putexcel set "$dir_work/reg_employmentSelection_${country}", sheet("Info") replace
...
putexcel B14 = "Regions: EL3 = Attika (omitted), EL4 = Aegean Islands, EL7 = Central and Northern Greece"
...
global wage_eqn "lwage_hour dag dagsq i.deh_c3 i.deh_c3#c.dag i.dhe i.drgn1 i.pt real_wage_growth Y2013-Y2021 Y2022_2023"
global seln_eqn "i.L1les_c3 dag dagsq i.deh_c3 i.deh_c3#c.dag i.mar i.child i.dhe i.drgn1 Y2012-Y2021 Y2022_2023" 
...
* Second stage
putexcel set "$dir_data/reg_wages_${country}", sheet("Females_NLW") replace
...
putexcel set "$dir_data/reg_wages_${country}", sheet("Females_NLW") modify
putexcel A1 = matrix(nonzero_b'), names nformat(number_d2) 

preserve

import excel "$dir_data/reg_wages_${country}", sheet("Females_NLW") firstrow ///
	clear

drop if C == 0 // UPDATE 
drop A 
drop AB-BB // UPDATE - include IMR

mkmat *, matrix(Females_NLW)
putexcel set "$dir_work/reg_wages_${country}", ///
	sheet("${country}_Wages_FemalesNE") modify 
...
local var_list Dag Dag_sq Deh_c3_Medium Deh_c3_Low Deh_c3_Medium_Dag ///
	...
	EL4 EL7 Pt RealWageGrowth Y2013 Y2014 Y2015 Y2016 Y2017 Y2018 Y2019 Y2020 Y2021 Y2022_2023 Constant InverseMillsRatio
...
putexcel set "$dir_work/reg_employmentSelection_${country}", ///
	sheet("${country}_EmploymentSelection_FemaleNE") modify 
putexcel B2 = matrix(Females_NLW)
...
local var_list Les_c3_NotEmployed_L1 Dag Dag_sq ///
	...
	Dhe_VeryGood Dhe_Excellent EL4 EL7 Y2012 Y2013 Y2014 Y2015 Y2016 Y2017 Y2018 Y2019 Y2020 Y2021 Y2022_2023 ///
	Constant	
...
* Second stage
putexcel set "$dir_data/reg_wages_${country}", sheet("Males_NLW") modify
putexcel C2 = matrix(var)
...
import excel "$dir_data/reg_wages_${country}", sheet("Males_NLW") firstrow clear
...
putexcel set "$dir_work/reg_wages_${country}", ///
	sheet("${country}_Wages_MalesNE") modify 
...
mkmat *, matrix(Males_NLW)
putexcel set "$dir_work/reg_employmentSelection_${country}", ///
	sheet("${country}_EmploymentSelection_MaleNE") modify
putexcel B2 = matrix(Males_NLW)
...
global wage_eqn "lwage_hour L1.lwage_hour dag dagsq i.deh_c3 i.deh_c3#c.dag i.dhe i.drgn1 i.pt real_wage_growth Y2013-Y2021 Y2022_2023" 
global seln_eqn "dag dagsq i.deh_c3 i.deh_c3#c.dag i.mar i.child i.dhe i.drgn1 Y2012-Y2021 Y2022_2023" 
...
* Second stage
putexcel set "$dir_data/reg_wages_${country}", sheet("Females_LW") modify
putexcel C2 = matrix(var)
...
import excel "$dir_data/reg_wages_${country}", sheet("Females_LW") firstrow ///
	clear

drop if C == 0 // UPDATE
drop A 
drop AC-BB // UPDATE - keep IMR

mkmat *, matrix(Females_LW)
putexcel set "$dir_work/reg_wages_${country}", ///
	sheet("${country}_Wages_FemalesE") modify 
...
local var_list L1_log_hourly_wage Dag Dag_sq Deh_c3_Medium Deh_c3_Low ///
	...
	Dhe_Excellent EL4 EL7 Pt RealWageGrowth Y2013 Y2014 Y2015 Y2016 Y2017 Y2018 Y2019 Y2020 Y2021 Y2022_2023 Constant ///
	InverseMillsRatio
...
mkmat *, matrix(Females_LW)
putexcel set "$dir_work/reg_employmentSelection_${country}", ////
	sheet("${country}_EmploymentSelection_FemaleE") modify 
putexcel B2 = matrix(Females_LW)
...
* Second stage 
putexcel set "$dir_data/reg_wages_${country}", sheet("Males_LW") modify
putexcel C2 = matrix(var)
...
import excel "$dir_data/reg_wages_${country}", sheet("Males_LW") firstrow clear
...
putexcel set "$dir_work/reg_wages_${country}", ///
	sheet("${country}_Wages_MalesE") modify 
...
mkmat *, matrix(Males_LW)
putexcel set "$dir_work/reg_employmentSelection_${country}", ///
	sheet("${country}_EmploymentSelection_MaleE") modify 
putexcel B2 = matrix(Males_LW)
