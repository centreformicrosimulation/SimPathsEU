...
* Only include adults in the sample 
drop if dag < 16
...
* Activity status if in education last year 
preserve

keep if dag >= 16 & dag <= 29 & l.ded == 1
...
(line retired year, sort color(grey) ///
	legend(label(4 "Retired"))) ///	
, title("Activity status") xtitle("Year") ytitle("Share") ///
	note(Notes: Age 16 - 29 and in initial education spell last year)
...
* Set Excel file 
* Info sheet
putexcel set "$dir_work/reg_education_IT_raw", sheet("Info") replace
...
putexcel set "$dir_work/reg_education_${country}_raw", sheet("Gof") modify
...
mkmat v*, matrix(var)	
putexcel set "$dir_work/reg_education_${country}_raw", sheet("E1a") modify
...
local var_list Dgn Dag Dag_sq ITF ITG ITH ITI  Year_transformed Y2020 Y2021 ///
	Constant
...
mkmat v*, matrix(var)	
putexcel set "$dir_work/reg_education_${country}_raw", sheet("E1b") modify
putexcel C2 = matrix(var)
...
local var_list Dgn Dag Dag_sq Deh_c3_High_L1 Deh_c3_Low_L1 ///
	Les_c3_NotEmployed_L1 Dnc_L1 Dnc02_L1 ITF ITG ITH ITI  Year_transformed ///
...
gologit2 deh_c3_recoded i.dgn dag dagsq i.drgn1 stm y2020 y2021  if ///
	...
	[pweight = dimxwt], autofit 	// removed time trend 
...
mkmat v*, matrix(var)	
putexcel set "$dir_work/reg_education_${country}_raw", sheet("E2a_raw") modify 
 ...
 * Labelling 
local no_coefs = 11 // number of coefs estimated per category
putexcel A2 = "CAT"
...
forvalues i = 0/1 {
	local n = 12 + `i'*`no_coefs'
	putexcel B`n' = "Y2021"
}

forvalues i = 0/1 {
	local n = 13 + `i'*`no_coefs'
