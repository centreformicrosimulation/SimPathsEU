...
do "$dir_do/extra_var_info/vars_05_20_HU2.do"
...
count if age_dif != 0 & age_dif != 1 & age_dif != . // 88 obs 

replace age_dif = 0 if age_dif == . 
bysort idperson (age_dif): egen temp_max_age_dif = max(age_dif)  
gen temp_age_issue = (temp_max_age_dif > 1)  

drop temp_max_age_dif age_dif dag_flag

lab var temp_age_issue "Indivdual has inconsistent age across panel"	

fre dag 
bys swv: sum dag 
	
/* Method 1: this option creates 149 missing values in the HU data and so we 
...
keep swv idperson dag  temp_age_i

rename idperson idpartner
rename dag dagsp 
rename temp_age_issue temp_age_issue_sp
...
--------------------+--------------------------------------------
Valid   1 Very good |      47190      12.14      14.77      14.77
        2 Good      |     122148      31.42      38.24      53.01
        3 Fair      |      94979      24.43      29.73      82.74
        4 Bad       |      42236      10.86      13.22      95.97
        5 Very bad  |      12889       3.32       4.03     100.00
        Total       |     319442      82.17     100.00           
Missing .           |      69316      17.83                      
Total               |     388758     100.00                      
-----------------------------------------------------------------
...
bys les_c3: fre lhw 
	// AB: 918 cases with positive hours of work who are not employed 
	// AB: 1709 cases of missing les_c3 info but positive hours of work recorded
sort idperson swv 
...
*/
/*State Retirement Ages for Men in the HU (2009-2024):
...
State Retirement Ages for Women in the HU (2005-2023):
...
Ages are approximate, Hungary had various options for taking early retirement 
and different sources had different ages.
*/
gen dagpns = 0

* Men
replace dagpns = 1 if dgn == 1 & dag >= 62 & stm >= 2005 & stm < 2014
replace dagpns = 1 if dgn == 1 & dag >= 63 & stm >= 2014 & stm < 2017 
replace dagpns = 1 if dgn == 1 & dag >= 64 & stm >= 2017 & stm <= 2020 
replace dagpns = 1 if dgn == 1 & dag >= 65 & stm >= 2021 & stm <= 2023 

* Women 
replace dagpns = 1 if dgn == 0 & dag >= 60 & stm >= 2005 & stm < 2010
replace dagpns = 1 if dgn == 0 & dag >= 62 & stm >= 2010 & stm < 2014
replace dagpns = 1 if dgn == 0 & dag >= 63 & stm >= 2014 & stm < 2017
replace dagpns = 1 if dgn == 0 & dag >= 64 & stm >= 2017 & stm <= 2020
replace dagpns = 1 if dgn == 0 & dag >= 65 & stm >= 2021 & stm <= 2023 
...
* Men 
replace dagpns_y = 1 if dgn == 1 & dag == 62 & stm >= 2005 & stm < 2014
replace dagpns_y = 1 if dgn == 1 & dag == 63 & stm >= 2014 & stm < 2017 
replace dagpns_y = 1 if dgn == 1 & dag == 64 & stm >= 2017 & stm <= 2020 
replace dagpns_y = 1 if dgn == 1 & dag == 65 & stm >= 2021 & stm <= 2023 

* Women
replace dagpns_y = 1 if dgn == 0 & dag == 60 & stm >= 2005 & stm < 2010
replace dagpns_y = 1 if dgn == 0 & dag == 62 & stm >= 2010 & stm < 2014
replace dagpns_y = 1 if dgn == 0 & dag == 63 & stm >= 2014 & stm < 2017
replace dagpns_y = 1 if dgn == 0 & dag == 64 & stm >= 2017 & stm <= 2020
replace dagpns_y = 1 if dgn == 0 & dag == 65 & stm >= 2021 & stm <= 2023 

* Became eligable for state pension last year 
gen dagpns_y1 = 0 

* Men 
replace dagpns_y1 = 1 if dgn == 1 & dag == 63 & stm >= 2005 & stm < 2014
replace dagpns_y1 = 1 if dgn == 1 & dag == 64 & stm >= 2014 & stm < 2017 
replace dagpns_y1 = 1 if dgn == 1 & dag == 65 & stm >= 2017 & stm <= 2020 
replace dagpns_y1 = 1 if dgn == 1 & dag == 66 & stm >= 2021 & stm <= 2023 

* Women
replace dagpns_y1 = 1 if dgn == 0 & dag == 61 & stm >= 2005 & stm < 2010
replace dagpns_y1 = 1 if dgn == 0 & dag == 63 & stm >= 2010 & stm < 2014
replace dagpns_y1 = 1 if dgn == 0 & dag == 64 & stm >= 2014 & stm < 2017
replace dagpns_y1 = 1 if dgn == 0 & dag == 65 & stm >= 2017 & stm <= 2020
replace dagpns_y1 = 1 if dgn == 0 & dag == 66 & stm >= 2021 & stm <= 2023 
...
/************************** HU GENERAL FERTILITY RATE *************************/

/* General fertility rate in Hungary covers the age range 15-49, statistics are 
...
fre yplgrs if yplgrs < 0 // 4703 obs
...
* Check for missing values == if missing on all the components 
count if py010g >= . & py050g >= .  & dag >= 16 // 12,177 adules missing both 
count if (py010g >= . & py050g >= . ) & dag >= 16 & les_c3 == 1 
	// 626 employed adults missing information 
...
replace CPI = 68.98  if stm == 2005
replace CPI = 71.76  if stm == 2006
replace CPI = 77.45  if stm == 2007
replace CPI = 82.12  if stm == 2008
replace CPI = 85.43  if stm == 2009
replace CPI = 89.47  if stm == 2010
replace CPI = 92.98  if stm == 2011
replace CPI = 98.24  if stm == 2012
replace CPI = 99.92  if stm == 2013
replace CPI = 99.94  if stm == 2014
replace CPI = 100    if stm == 2015
replace CPI = 100.45 if stm == 2016
replace CPI = 102.84 if stm == 2017
replace CPI = 105.84 if stm == 2018
replace CPI = 109.46 if stm == 2019
replace CPI = 113.15 if stm == 2020
replace CPI = 119.04 if stm == 2021
replace CPI = 137.22 if stm == 2022
replace CPI = 160.59 if stm == 2023
...
-------------+---------------------------------------------------------
  ypnbihs_dv |    388,758     3.21644    3.263202          0      12.57
ypnbihs_dv~p |    180,235    4.334182    3.190466          0      12.57
 yptciihs_dv |    388,758    .5128789    1.342981          0   9.787335
   yplgrs_dv |    388,758    2.976737    3.354051         -9      12.57
...
/*
    Variable |        Obs        Mean    Std. dev.       Min        Max
-------------+---------------------------------------------------------
 yhhnb_asinh |    388,758    3.556406    3.263162          0   11.73983
...
count if hy080g >= . | hy110g >= . |  hy040g >= . |  hy090g >= . 
	// 43 obs have some missing capital income information 
...
For Hungary, py130g is better recorded than py130n, and py132g & py134g also 
contain information, whereas py131g and py133g only have missings. 

The code below may well be HU specific as some of the coding of these variables 
...
------------+-----------------------------------
          1 |      6,177       20.81       20.81
          2 |     16,997       57.27       78.09
          4 |      6,503       21.91      100.00
------------+-----------------------------------
      Total |     29,677      100.00
...
------------+-----------------------------------
          1 |      5,676       19.68       19.68
          2 |     10,928       37.89       57.57
          3 |      6,275       21.76       79.33
          4 |      5,962       20.67      100.00
------------+-----------------------------------
      Total |     28,841      100.00
