********************************************************************************
* PROJECT:              ESPON
* DO-FILE NAME:         04_reweight_EL.do
...
* 1. Adjust weights using the inverse probability of not having any missing 
* values 
...
* Restrict sample to observations without missing variables 
...
/*
    ...
    -----------------------------------------
    Not matched                        30,730
        from master                    30,730  (_merge==1)
        from using                          0  (_merge==2)

    Matched                           389,269  (_merge==3)
    -----------------------------------------
*/	
...
foreach vv of varlist dwt drgn1 dhhtp_c4 ydses_c5 dnc02 dnc {
	...
	bys stm idbenefitunit: replace `vv' = `vv'[1] if (`vv'!=`vv'[1])

}
	
save "$dir_data/${country}-SILC_pooled_all_obs_04.dta", replace  

* 4. Slice the original pooled dataset into years 
