...
Dataset quirk: For Italy the data collection wave adn personal interview year 
don't align. Therefore utilize hb010 (household data year of survey) instead of 
year of personal interview as it alligns perfectly with year. This wasn't the 
case for PL or HU, adn so should use personal interview where posssible. 

tab year if year != pb010 


       year |      Freq.     Percent        Cum.
------------+-----------------------------------
       2004 |      7,354        3.85        3.85
       2005 |     23,091       12.09       15.94
       2006 |     21,104       11.05       26.99
       2007 |     19,496       10.21       37.19
       2008 |     18,768        9.82       47.02
       2009 |      8,830        4.62       51.64
       2010 |      7,704        4.03       55.67
       2011 |      8,654        4.53       60.20
       2012 |      8,212        4.30       64.50
       2013 |      7,198        3.77       68.27
       2014 |      7,527        3.94       72.21
       2015 |      7,231        3.79       75.99
       2016 |      7,398        3.87       79.87
       2017 |      6,946        3.64       83.50
       2018 |      6,583        3.45       86.95
       2019 |      5,909        3.09       90.04
       2020 |      4,274        2.24       92.28
       2021 |      4,323        2.26       94.54
       2022 |      5,569        2.92       97.46
       2023 |      4,855        2.54      100.00
------------+-----------------------------------
      Total |    191,026      100.00

Mainly impacts years we are not interested in. 
...
count if swv != stm //65,992 

gen diff = abs(swv - stm)
tab diff // if off, off by one year 

gen diff_dir = swv - stm
tab diff_dir // if off the personal interview happened the following yr

sort upid year 
gen diff_double = stm - swv[_n+1] if upid == upid[_n+1]
tab diff_double if diff == 1, mis
/*
Most would generate double counts of the following year if used stm 
Also 43% missing observations which are the final observations for the ind
*/ 
...
duplicates drop swv idperson, force //(833 observations deleted)
xtset idperson swv 
...
/*
"Similarly, the old ITD5 region (Emilia-Romagna) has a correspondence with the 
new ITH5 region with the same name. The only difference between ITD5 and ITH5 
lies in that its ninth NUTS 3 region, ITD59 (Rimini), experienced a boundary 
shift, being transformed into the bigger ITH59 (Rimini). This boundary shift is 
in turn compensated by the complementary reduction in the old ITE31 (Pessaro e 
Urbino), which was part of the old NUTS 2 ITE3 (Marche), which are converted 
into the new ITI31 (Pessaro e Urbino) and ITI3 (Marche) respectively."
From "Conversion of regional data between NUTS classifications" Lopez-Cobo 2016 

Agreed that there was a minimla change so in the data assume:
	ITD -> ITH  
	ITE -> ITI 

*/

clonevar drgn1_code = db040 

replace drgn1_code = subinstr(drgn1_code, "$country", "", .)

generate drgn1 = strpos("ABCDEFGHIJKLMNOPQRSTUVWXYZ", drgn1_code) 

tab drgn1 year  

replace drgn1 = 8 if drgn1 == 4 
replace drgn1 = 9 if drgn1 == 5 
...
fre dagsp if idpartner > 0  
	// 8 people in a relationship with a partner 16 or younger 
	// 38 people in a relationship with a partner 17 or younger
bys swv: sum dagsp 
	
/******************************* HEALTH STATUS ********************************/
/*
Data quirk: No health information for 2020. => impute 
*/
...
--------------------+--------------------------------------------
Valid   1 Very good |     101303      10.78      13.46      13.46
        2 Good      |     394627      42.01      52.45      65.91
        3 Fair      |     183084      19.49      24.33      90.24
        4 Bad       |      58834       6.26       7.82      98.06
        5 Very bad  |      14571       1.55       1.94     100.00
        Total       |     752419      80.10     100.00           
Missing .           |     186973      19.90                      
Total               |     939392     100.00                      
-----------------------------------------------------------------
...
fre deh_c3	// 5% missing
fre dag if deh_c3 == -9  
fre year if deh_c3 == -9  
tab deh_c3 year, col mis //missing 25% of 2020 
bys swv: sum deh_c3 if deh_c3 > 0 

count if deh_c3 == -9  // 11,517
...
fre dehsp_c3 if idpartner > 0 // 4.43% missing 
...
fre dcpagdf // 51% missing  
tab dcpagdf year, col
bys swv: sum dcpagdf

/*
There seems to be a slight issue in 2023 as postiive and negative max 
differences are not the same in abs value. Eye-balling suggests not systematic 
*/
...
tab dlltsd les_c3
count if dlltsd == 1 & les_c3 == . // 315 due to imputation 
...
CountryProfile?primaryCountry=ITA&treshold=10&topic=EO) 
*/
sort idperson swv 
xtset idperson swv
gen ded = 0 
...
bys les_c3: fre lhw 
	// AB: 3135 cases with positive hours of work who are not employed 
	// AB: 2839 cases of missing les_c3 info but positive hours of work recorded
sort idperson swv 
...
Contradictions with the retired appear to be due to making retirement an 
absorbing state. 
...
Decision: Because activity status is more important, prioritize that information
=> If activity status implies not working, give zero workign hours 
=> If activity status says shoudl be working but missing working hrs info,  
...
* Address missing values of tthe employed with missing hours info 
...
tab les_c3 ft if year > 2008 // contradictions all due to retirement absorbing 
...
tab les_c4 unemp // discrepancy caused by imputing retirement 
...
*/
/*State Retirement Ages for Men in the IT (2009-2024):
...
State Retirement Ages for Women in the IT (2005-2023):
...
Ages are approximate, Italy had various options for taking early retirement 
and different sources had different ages.
*/
gen dagpns = 0

* Men
replace dagpns = 1 if dgn == 1 & dag >= 65 & stm >= 2005 & stm < 2012
replace dagpns = 1 if dgn == 1 & dag >= 66 & stm >= 2012 & stm < 2019 
replace dagpns = 1 if dgn == 1 & dag >= 67 & stm >= 2019 & stm <= 2023 
 

* Women 
replace dagpns = 1 if dgn == 0 & dag >= 60 & stm >= 2005 & stm < 2012
replace dagpns = 1 if dgn == 0 & dag >= 62 & stm >= 2012 & stm < 2013
replace dagpns = 1 if dgn == 0 & dag >= 63 & stm >= 2013 & stm < 2015
replace dagpns = 1 if dgn == 0 & dag >= 65 & stm >= 2015 & stm < 2018
replace dagpns = 1 if dgn == 0 & dag >= 66 & stm >= 2018 & stm < 2019
replace dagpns = 1 if dgn == 0 & dag >= 67 & stm >= 2019 & stm <= 2023 
...
* Men 
replace dagpns_y = 1 if dgn == 1 & dag == 65 & stm >= 2005 & stm < 2012
replace dagpns_y = 1 if dgn == 1 & dag == 66 & stm >= 2012 & stm < 2019 
replace dagpns_y = 1 if dgn == 1 & dag == 67 & stm >= 2019 & stm <= 2023 


* Women
replace dagpns_y = 1 if dgn == 0 & dag == 60 & stm >= 2005 & stm < 2012
replace dagpns_y = 1 if dgn == 0 & dag == 62 & stm >= 2012 & stm < 2013
replace dagpns_y = 1 if dgn == 0 & dag == 63 & stm >= 2013 & stm < 2015
replace dagpns_y = 1 if dgn == 0 & dag == 65 & stm >= 2015 & stm < 2018
replace dagpns_y = 1 if dgn == 0 & dag == 66 & stm >= 2018 & stm < 2019
replace dagpns_y = 1 if dgn == 0 & dag == 67 & stm >= 2019 & stm <= 2023 

* Became eligable for state pension last year 
gen dagpns_y1 = 0 

* Men 
replace dagpns_y1 = 1 if dgn == 1 & dag == 66 & stm >= 2005 & stm < 2012
replace dagpns_y1 = 1 if dgn == 1 & dag == 67 & stm >= 2012 & stm < 2019 
replace dagpns_y1 = 1 if dgn == 1 & dag == 68 & stm >= 2019 & stm <= 2023 


* Women
replace dagpns_y1 = 1 if dgn == 0 & dag == 61 & stm >= 2005 & stm < 2012
replace dagpns_y1 = 1 if dgn == 0 & dag == 63 & stm >= 2012 & stm < 2013
replace dagpns_y1 = 1 if dgn == 0 & dag == 64 & stm >= 2013 & stm < 2015
replace dagpns_y1 = 1 if dgn == 0 & dag == 65 & stm >= 2015 & stm < 2018
replace dagpns_y1 = 1 if dgn == 0 & dag == 66 & stm >= 2018 & stm < 2019
replace dagpns_y1 = 1 if dgn == 0 & dag == 68 & stm >= 2019 & stm <= 2023
...
/*************************** WOMEN AGED 18 - 49 *******************************/
gen sprfm = 0
replace sprfm = 1 if dgn == 0 &  dag >= 18 & dag <= 49
...
count if py010g >= . &  py050g >= . &  py080g >= . & hy080g >= . & ///
	hy110g >= . & hy040g >= . & hy090g >= . // 158,315 obs 
	
count if py010g >= . &  py050g >= . &  py080g >= . & hy080g >= . & ///
	hy110g >= . & hy040g >= . & hy090g >= .	& dag >= 16 // 135,230
	
tab year if py010g >= . &  py050g >= . &  py080g >= . & hy080g >= . & ///
	hy110g >= . & hy040g >= . & hy090g >= .	& dag >= 16	
/*	
All obs missing all income info are from 2004-2006, which we do not use
therefore not a problem. 
*/	
count if (py010g >= . |  py050g >= . | py080g >= . | hy080g >= . | ///
	hy110g >= . | hy040g >= . | hy090g >= .) & dag >= 16 //  153,523 

tab year  if (py010g >= . |  py050g >= . | py080g >= . | hy080g >= . | ///
	hy110g >= . | hy040g >= . | hy090g >= .) & dag >= 16 
/*
Lionshare of these are from 2004-2006; 18,293 from after 2006
*/	
	
count if dag >= 16 // 332,907
//=> 20% of adult observations have some missing all income information 
...
count if py080g >= . & hy080g >= . & hy110g >= . & hy040g >= . & ///
	hy090g >= . // 158,315 obs with all missing elements  

count if (py080g >= . | hy080g >= . | hy110g >= . | hy040g >= . | ///
	hy090g >= .) & dag >= 16 
	// 153,523 adult obs with at least one missing element 
...
* Check for missing values == if missing on all the components 
count if py010g >= . & py050g >= .  & dag >= 16 // 153,523  
count if (py010g >= . & py050g >= . ) & dag >= 16 & les_c3 == 1 
	// 58,810 employed adults missing information 
...
replace CPI = 83.7  if stm == 2005
replace CPI = 85.6  if stm == 2006
replace CPI = 87.3  if stm == 2007
replace CPI = 90.4  if stm == 2008
replace CPI = 91.1  if stm == 2009
replace CPI = 92.6  if stm == 2010
replace CPI = 95.3  if stm == 2011
replace CPI = 98.4  if stm == 2012
replace CPI = 99.7  if stm == 2013
replace CPI = 99.9  if stm == 2014
replace CPI = 100   if stm == 2015
replace CPI = 99.9  if stm == 2016
replace CPI = 101.3 if stm == 2017
replace CPI = 102.5 if stm == 2018
replace CPI = 103.2 if stm == 2019
replace CPI = 103.0 if stm == 2020
replace CPI = 105.0 if stm == 2021
replace CPI = 114.2 if stm == 2022
replace CPI = 120.9 if stm == 2023
...
-------------+---------------------------------------------------------
  ypnbihs_dv |    892,803    3.928208    3.740359          0   13.63781
ypnbihs_dv~p |    433,752    4.809737    3.672086          0   13.63781
 yptciihs_dv |    892,803    1.737594    2.349341          0   10.94229
   yplgrs_dv |    892,803    2.676194    4.652311         -9   13.63754
...
/*
    Variable |        Obs        Mean    Std. dev.       Min        Max
-------------+---------------------------------------------------------
 yhhnb_asinh |    892,803    4.285993    3.773225          0   13.23481
...
* Check for missing values == if missing on all the components 
count if hy080g >= . & hy110g >= . &  hy040g >= . &  hy090g >= . //  158,315
count if hy080g >= . | hy110g >= . |  hy040g >= . |  hy090g >= . 
	//  158,315 obs have some missing capital income information 
...
count if yplgrs_annual > 0 & months_wrk_missing != 0 //86,346
/*
 => many missing values 
...
* EUROMOD weight based on DB090, sums up to the population of IT, see below: 
/*	
...
------------+-----------------------------------
          1 |     15,805       27.87       27.87
          2 |     13,451       23.72       51.60
          3 |     13,624       24.03       75.62
          4 |     13,822       24.38      100.00
------------+-----------------------------------
      Total |     56,702      100.00
...
-> stm = 2006
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     14,082       25.59       25.59
          2 |     15,389       27.97       53.56
          3 |     12,700       23.08       76.64
          4 |     12,853       23.36      100.00
------------+-----------------------------------
      Total |     55,024      100.00
...
-> stm = 2007
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     12,734       23.90       23.90
          2 |     13,459       25.26       49.16
          3 |     15,302       28.72       77.88
          4 |     11,786       22.12      100.00
------------+-----------------------------------
      Total |     53,281      100.00
...
-> stm = 2008
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     11,745       22.16       22.16
          2 |     12,368       23.33       45.49
          3 |     13,627       25.71       71.20
          4 |     15,263       28.80      100.00
------------+-----------------------------------
      Total |     53,003      100.00
...
-> stm = 2009
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     14,626       28.29       28.29
          2 |     11,216       21.70       49.99
          3 |     12,593       24.36       74.35
          4 |     13,258       25.65      100.00
------------+-----------------------------------
      Total |     51,693      100.00
...
-> stm = 2010
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     11,796       24.64       24.64
          2 |     13,024       27.20       51.84
          3 |     11,055       23.09       74.93
          4 |     12,001       25.07      100.00
------------+-----------------------------------
      Total |     47,876      100.00
...
-> stm = 2011
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     11,002       22.54       22.54
          2 |     10,556       21.62       44.16
          3 |     16,350       33.49       77.65
          4 |     10,910       22.35      100.00
------------+-----------------------------------
      Total |     48,818      100.00
...
------------+-----------------------------------
          1 |      9,672       20.02       20.02
          2 |      9,367       19.39       39.42
          3 |     13,332       27.60       67.02
          4 |     15,931       32.98      100.00
------------+-----------------------------------
      Total |     48,302      100.00
...
-> stm = 2013
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     14,398       31.89       31.89
          2 |      7,736       17.14       49.03
          3 |     11,016       24.40       73.43
          4 |     11,992       26.57      100.00
------------+-----------------------------------
      Total |     45,142      100.00
...
-> stm = 2014
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     11,464       24.00       24.00
          2 |     14,673       30.72       54.72
          3 |     10,311       21.59       76.31
          4 |     11,313       23.69      100.00
------------+-----------------------------------
      Total |     47,761      100.00
...
-> stm = 2015
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     10,472       23.93       23.93
          2 |     12,118       27.69       51.61
          3 |     10,920       24.95       76.56
          4 |     10,260       23.44      100.00
------------+-----------------------------------
      Total |     43,770      100.00
...
-> stm = 2016
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     10,058       20.55       20.55
          2 |     10,975       22.42       42.97
          3 |      8,646       17.67       60.64
          4 |     19,264       39.36      100.00
------------+-----------------------------------
      Total |     48,943      100.00
...
-> stm = 2017
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     15,817       32.06       32.06
          2 |     10,202       20.68       52.74
          3 |      8,482       17.19       69.93
          4 |     14,837       30.07      100.00
------------+-----------------------------------
      Total |     49,338      100.00
...
-> stm = 2018
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     12,568       27.00       27.00
          2 |     12,851       27.61       54.61
          3 |      7,270       15.62       70.23
          4 |     13,857       29.77      100.00
------------+-----------------------------------
      Total |     46,546      100.00
...
-> stm = 2019
 ...
 Rotational |
      group |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |     10,591       23.94       23.94
          2 |      8,777       19.84       43.78
          3 |     12,995       29.38       73.16
          4 |     11,873       26.84      100.00
------------+-----------------------------------
      Total |     44,236      100.00
...
------------+-----------------------------------
          ...
          2 |      3,657        8.06       18.37
          3 |      4,302        9.48       27.85
          4 |     21,275       46.89       74.74
          5 |      5,436       11.98       86.73
          6 |      6,023       13.27      100.00
------------+-----------------------------------
      Total |     45,372      100.00
...
------------+-----------------------------------
          2 |      3,791        9.85        9.85
          3 |      4,502       11.69       21.54
          4 |     17,331       45.02       66.56
          5 |      6,000       15.58       82.14
          6 |      6,876       17.86      100.00
------------+-----------------------------------
      Total |     38,500      100.00
