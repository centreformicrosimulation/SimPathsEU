...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg eqdisp_inc_`year'_all eqdisp_inc_`year'_ZERO ///
		...
		graphregion(color(white)) ///		
		note("Notes: Distribution of benefit unit equivalised disposable income. Individual level data plotted 18-65 year olds included in sample. Values in" "€ per year, 2015 prices. Top and bottom percentiles trimmed. Weekly hours worked categories:" "ZERO = 0, TWENTY = [1,39], FORTY = 40, FIFTY = 41+", ///
		size(vsmall)) 
