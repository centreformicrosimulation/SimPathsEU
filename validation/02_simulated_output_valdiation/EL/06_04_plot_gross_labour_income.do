...
(line valid_y_gross_labour_yr_bu year, sort color(green) ///
	...
	note("Note: Amount at the benefit unit level, individual data plotted. Statistics calculated on the sample of employed individuals" "ages 18-65. 2019 X-sectional data used in underlying estimation. Amounts in 2015 prices. Top and bottom percentiles trimmed.", ///
	size(vsmall))
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg gross_labour_inc_`year'_all  ///
		...
		graphregion(color(white)) ///
		note("Notes: Amount at the benefit unit level, individual data plotted. Values in € per year, 2015 prices. 18-65 years olds included in the sample." "Top and bottom percentiles trimmed. Weekly hours worked categories: ZERO = 0, TWENTY = [1,39], FORTY = 40, FIFTY = 41+.", ///
		size(vsmall)) 
