...
********************************************************************************
* 1 : Mean values over time  (gross income of individual)
********************************************************************************	
...
* Male 
* Prepare validation data 
use year dwt valid_y_gross_nsbc_person_yr dgn using ///
	"$dir_data/${country}-eusilc_validation_sample.dta", clear
...
* Prepare validation data 
use year dwt valid_y_gross_nsbc_yr_bu laboursupplyweekly_hu using ///
	"$dir_data/${country}-eusilc_validation_sample.dta", clear
...
forval year = `min_year'/`max_year' {
	...
	* Prepare info needed for dynamic y axis labels
	...
	(hist valid_y_gross_nsbc_yr_bu if year == `year', width(750) ///
...
forval year =  `min_year'/`max_year' { 
	...
	* Plot all hours
	twoway (hist sim_y_gross_yr if year == `year', width(750) ///
		color(green%30) legend(label(1 "Simulated"))) ///
	(hist valid_y_gross_nsbc_person_yr if year == `year', width(750) ///
		color(red%30) legend(label(2 "Observed"))) , ///
