...
* NOTES: 			This file produces eusilc_validation_sample.dta file which 
* 					provides validation targets for the simulated data.
* 					In the UKHLS version children <16 were dropped because they 
* 					did not have weights ==> in EUSILC we will use household 
* 					weight which is defined for everyone so children will remain
*					in the sample  
********************************************************************************
clear all
...
forvalues year = 11/23 { //beginning of year loop 
	...
	* Household Register (D-FILE)
	global d_file ///
	"${EUSILC_original_crosssection}\\${country}\20${silc_yr}\UDB_c${country}${silc_yr}D.csv"		

	* Personal Register (R-FILE), i.e. all people
	global r_file ///
	"${EUSILC_original_crosssection}\\${country}\20${silc_yr}\UDB_c${country}${silc_yr}R.csv"		

	* Household Data (H-FILE)
	global h_file ///
	"${EUSILC_original_crosssection}\\${country}\20${silc_yr}\UDB_c${country}${silc_yr}H.csv"		

	* Personal Data (P-FILE), i.e. people aged 16 and above
	global p_file ///
	"${EUSILC_original_crosssection}\\${country}\20${silc_yr}\UDB_c${country}${silc_yr}P.csv"		
...
forvalues year = 11/23 { //beginning of year loop 
	...
	************************************************************************
	...
	di in y ///
		"Number of observations where gender is invalid or missing (dgn): " r(N)
	if (r(N) > 0) noi di in r "MUST BE IMPUTED!"
	...
	* Children coded as "Never Married" (age <= 17)
	...
	* Merge regions as in initial population and estimates 
	replace drgn1 = 7 if drgn1 == 5 
	replace drgn1 = 7 if drgn1 == 6 
	
	lab var drgn1 "Region"
	lab define drgn1 ///
	3 "Attica" ///
	4 "Aegean Islands" ///
	7 "Northern and Central Greece"
