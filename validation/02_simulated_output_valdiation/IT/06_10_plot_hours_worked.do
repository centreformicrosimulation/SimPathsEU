/*
This do file plots simulated and observed hours of work of employed individuals
...
(line hours year, sort color(green) legend(label(2 "Observed"))), ///
	title("Average weekly hours worked") xtitle("Year") ytitle("Hours per week") ///
	ylabel(34 [2] 44) ///
	graphregion(color(white)) ///
	note("Note: Statistics calculated on sample of employed and self-employed individuals", size(vsmall))		
...
forval year = `min_year'/`max_year' {
	...
	* Prepare info needed for dynamic y axis labels 
	...
	twoway (hist lhw if year == `year' & lhw <= 65, color(green%20) ///
		legend(label(1 "Simulated"))) ///
	(hist hours if year == `year' & hours <= 65, color(red%20) ///
	...
	graph export ///
		"$dir_output_files/hours_worked/validation_${country}_hours_worked_dist_`year'.png", ///
