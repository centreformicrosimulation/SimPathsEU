/*
This do file plots simulated and observed disposable income, per benefit unit
...
use year dwt valid_y_disp_yr_bu using ///
...
title("Disposable income") xtitle("Year") ytitle("€ per year") ///
	ylabel(,labsize(small)) xlabel(,labsize(small)) ///
	graphregion(color(white)) ///
	note("Notes: Statistics computed at the benefit unit level. Sample includes all individuals ages 18-65. Amounts in 2015 prices. ", size(vsmall))

graph export ///
"$dir_output_files/income/validation_${country}_disposable_income_ts_${min_age}_${max_age}_both.jpg", ///
...
* Prepare validation data
use year dwt valid_y_disp_yr_bu dgn using ///
	"$dir_data/${country}-eusilc_validation_sample.dta", clear
...
* Prepare simulated data
use run year sim_y_disp_yr_bu dgn using "$dir_data/simulated_data.dta", clear
...
(line valid_y_disp_yr_bu year, sort color(green) ///
	legend(label(2 "Observed"))), ///
title("Disposable income") subtitle("Males") ///
	xtitle("Year") ytitle("€ per year") ///
	ylabel(,labsize(small)) xlabel(,labsize(small)) ///
	graphregion(color(white)) ///
	note("Notes: Statistics computed at the benefit unit level. Sample includes males ages 18-65. Amounts in 2015 prices. ", size(vsmall))

graph export ///
"$dir_output_files/income/validation_${country}_disposable_income_ts_${min_age}_${max_age}_male.jpg", ///
	replace //width(2560) height(1440) quality(100)
...
(line valid_y_disp_yr_bu year, sort color(green) ///
	legend(label(2 "Observed"))), ///
title("Disposable income") subtitle("Females") ///
	xtitle("Year") ytitle("€ per year") ///
	ylabel(,labsize(small)) xlabel(,labsize(small)) ///
	graphregion(color(white)) ///
	note("Notes: Statistics computed at the benefit unit level. Sample includes females ages 18-65. Amounts in 2015 prices. ", size(vsmall))

graph export ///
"$dir_output_files/income/validation_${country}_disposable_income_ts_${min_age}_${max_age}_female.jpg", ///
	replace //width(2560) height(1440) quality(100)	
...
use year dwt valid_y_disp_yr_bu laboursupplyweekly_hu dag dgn ///
	using "$dir_data/${country}-eusilc_validation_sample.dta", clear
...
forval year = `min_year'/`max_year' {
    ...
    * Prepare info needed for dynamic y axis labels 
	twoway__histogram_gen sim_y_disp_yr_bu if year == `year', bin(60) ///
		den gen(d_sim v1)
	...
	* Plot all hours
	twoway (hist sim_y_disp_yr_bu if year == `year' , color(green%30) ///
		legend(label(1 "Simulated"))) ///
	(hist valid_y_disp_yr_bu if year == `year' , color(red%30) ///
		legend(label(2 "Observed"))) , ///
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg disp_inc_`year'_all disp_inc_`year'_ZERO disp_inc_`year'_THIRTY ///
		disp_inc_`year'_THIRTY_SIX disp_inc_`year'_FORTY ///
		disp_inc_`year'_FIFTY, ///
		title("Disposable income by weekly hours of work") ///
		subtitle("`year', Males") ///		
		legendfrom(disp_inc_`year'_all) rows(2) ///
		graphregion(color(white)) ///		
		note("Notes: Sample includes all males aged 18-65. Values in € per year (2015 prices). Indiviudal level data for benefit level variable. Samples" "trimmed. Weekly hours worked categories: ZERO = 0, THIRTY = [1,35], THIRTY_SIX = [36, 39], FORTY = [40,49], FIFTY = 50+.", size(vsmall)) 
		
	graph export ///
	"$dir_output_files/income/validation_${country}_disposable_income_dist_`year'_male.png", ///
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg disp_inc_`year'_all disp_inc_`year'_ZERO disp_inc_`year'_TWENTY ///
		disp_inc_`year'_THIRTY disp_inc_`year'_THIRTY_SIX ///
		disp_inc_`year'_FORTY, ///
		title("Disposable income by weekly hours of work") ///
		subtitle("`year', Females") ///		
		legendfrom(disp_inc_`year'_all) rows(2) ///
		graphregion(color(white)) ///		
		note("Notes: Sample includes all females aged 18-65. Values in € per year (2015 prices). Indiviudal level data for benefit level variable. Samples" "trimmed. Weekly hours worked categories: ZERO = 0, TWENTY = [1,29], THIRTY = [30,35], THIRTY_SIX = [36, 39]. FORTY = 40+.", size(vsmall)) 
		
	graph export ///
	"$dir_output_files/income/validation_${country}_disposable_income_dist_`year'_female.png", ///
		replace width(2400) height(1350) 
}
