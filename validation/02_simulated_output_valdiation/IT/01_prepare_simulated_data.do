/*
This file pre-processes simulated output from SimPaths model. 

Author: Patryk Bronka
Last modified: DEC 2024 PB

*/

cd "${dir_simulated_data}"

* Import required variables from household file

import delimited Household.csv, clear 
keep run time id_household  
rename id_household idhousehold
keep if time <= ${max_year}

save household_validation, replace

* Import required variables from benefit unit file

import delimited BenefitUnit.csv, clear
keep run time idhousehold id_benefitunit disposableincome equivaliseddisposableincomeyearl grossincomemonthly dhhtp_c4
rename id_benefitunit idbenefitunit
keep if time <= ${max_year}	

save benefitunit_validation, replace

* Import required variables from person file

import delimited Person.csv, clear
keep run time id_person idpartner dcpst idbenefitunit dag les_c4 deh_c3 dhe ///
	dgn ypnbihs_dv yplgrs_dv ypncp ypnoab fulltimehourlyearningspotential ///
	laboursupplyweekly hoursworkedweekly dlltsd
rename id_person idperson
keep if time <= ${max_year}
										
save person_validation, replace

use person_validation, clear
merge m:1 run time idbenefitunit using benefitunit_validation, nogen ///
	keep(matched)
save baseline_validation, replace 
