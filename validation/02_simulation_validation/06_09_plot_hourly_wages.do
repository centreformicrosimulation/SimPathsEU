...
title("Hourly Wages") ///
	...
	note("Notes: Statistics calculated on sample of employed and self-employed individuals. Values in 2015 prices.", ///
	size(vsmall))
...
**** With min wage adjustment 

* Prepare validation data
use year dwt les_c4 valid_wage_hour_adj_mw using ///
	"$dir_data/${country}-eusilc_validation_sample_long.dta", clear

* Keep only employed individuals
keep if les_c4 == 1

drop if valid_wage_hour_adj == -9 

* Trim outliers
if "$trim_outliers" == "true" {
	
	sum valid_wage_hour_adj_mw, d
	replace valid_wage_hour_adj_mw = . if ///
		valid_wage_hour_adj_mw < r(p1) | valid_wage_hour_adj_mw > r(p99)

}

collapse (mean) valid_wage_hour_adj_mw [aw = dwt], by(year)
...
twoway (rarea potential_earnings_hourly_high ///
	potential_earnings_hourly_low year, ///
	sort color(green%20) legend(label(1 "Simulated"))) ///
(line valid_wage_hour_adj_mw year, sort color(green) ///
	legend(label(2 "Observed"))), ///
title("Hourly Wages") ///
	subtitle("Min wage adjusted") ///
	xtitle("Year", size(small)) ///
	ytitle("€ per year", size(small)) ///
	ylabel(,labsize(small)) ///
	xlabel(,labsize(small)) ///
...
* Save figure
graph export ///
"$dir_output_files/wages/validation_${country}_wages_ts_${min_age}_${max_age}_both_mw.jpg", ///
	replace width(2560) height(1440) quality(100)
	
	
**** Without those below the min wage 	

* Prepare validation data
use year dwt les_c4 valid_wage_hour_adj using ///
	"$dir_data/${country}-eusilc_validation_sample_long.dta", clear
...
if "$trim_outliers" == "true" {
	...
	sum valid_wage_hour_adj, d
	replace valid_wage_hour_adj = . if ///
		valid_wage_hour_adj < r(p1) | valid_wage_hour_adj > r(p99)

}

collapse (mean) valid_wage_hour_adj [aw = dwt], by(year)
...
twoway (rarea potential_earnings_hourly_high ///
	potential_earnings_hourly_low year, ///
	sort color(green%20) legend(label(1 "Simulated"))) ///
(line valid_wage_hour_adj year, sort color(green) ///
	legend(label(2 "Observed"))), ///
title("Hourly Wages") ///
	subtitle("Belwo min wage removed") ///
	xtitle("Year, Min wage adjusted", size(small)) ///
	ytitle("€ per year", size(small)) ///
	ylabel(,labsize(small)) ///
	xlabel(,labsize(small)) ///
	legend(size(small)) ///
