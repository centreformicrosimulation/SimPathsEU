...
* 					SimPaths model using SILC data. 
********************************************************************************
...
(line valid_wage year, sort color(green) ///
	...
	ytitle("Euro per hour", size(small)) ///
...
use year demAge dwt labC4 valid_wage demAge flag_wage_imp_panel ///
	flag_wage_hotdeck using "$dir_data/${country}-eusilc_validation_sample.dta", clear
...
forval year = `min_year'/`max_year' {
	...
	* Plot all hours
	twoway (hist sim_pred_wage if year == `year', ///
		width(0.5) color(green%30)  legend(label(1  "SimPaths"))) ///
	(hist valid_wage if year == `year', width(0.5)  color(red%30) ///
		legend(label(2 "SILC"))) , ///
		title("Hourly Wage") ///
		subtitle("`year'") ///
		name(hourly_wages_`year'_all, replace) ///
		xtitle("Euro per hour", size(small)) ///
