...
twoway ///
	...
	ytitle("Euro per year", size(small)) ///
...
forval year = `min_year'/`max_year' {
	...
	foreach ls in $ls_cat {
		...
		twoway__histogram_gen valid_yGrossBuLevelYear if ///
			year == `year' & valid_labHrsWorkEnumWeek == "`ls'", width(750) ///
			den gen(d_valid v2)
...
forval year = `min_year'/`max_year' {
	...
	* Prepare info needed for dynamic y axis labels
	...
	twoway (hist sim_yGrossBuLevelYear if year == `year', width(750) ///
		color(green%30) legend(label(1 "SimPaths"))) ///
	(hist valid_yGrossBuLevelYear if year == `year', width(750) ///
		color(red%30) legend(label(2 "SILC"))) , ///
		title("ALL hours") ///
		name(gross_inc_`year'_all, replace) ///
		xtitle("Euro", size(small)) ///
	...
	foreach ls in $ls_cat {
		...
		* Prepare info needed for dynamic y axis labels 	
		twoway__histogram_gen sim_yGrossBuLevelYear if ///
			year == `year' & sim_labHrsWorkEnumWeek == "`ls'", width(750) ///
			den gen(d_sim v1)
		...
		* Plot by weekly hours work
		twoway (hist sim_yGrossBuLevelYear if year == `year' & ///
			sim_labHrsWorkEnumWeek == "`ls'", width(750) color(green%30) ///
			legend(label(1 "SimPaths"))) ///
		(hist valid_yGrossBuLevelYear if year == `year' & ///
			valid_labHrsWorkEnumWeek == "`ls'", width(750) color(red%30) ///
			legend(label(2 "SILC"))) , ///
			title("`ls' hours") ///
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg gross_inc_`year'_all ///
		...
		gross_inc_`year'_FIFTY , ///		
		title("Benefit Unit Gross Income by Weekly Hours of Work") ///
		subtitle("`year'") ///
		legendfrom(gross_inc_`year'_ZERO) ///
		rows(2) ///
		graphregion(color(white)) ///
		note("Notes: Amounts in Euro per year, 2015 prices. Gross income is the sum of capital income, private pension income and employment income." "Individual observations of benefit unit amounts plotted.", ///
		size(vsmall)) 

	graph export "$dir_output_files/income/gross_income/validation_${country}_gross_income_bu_dist_`year'_18_65.png", ///
...
* Prepare validation data 
use year demAge dwt valid_yGrossBuLevelYear valid_labHrsWorkEnumWeek ///
	demMaleFlag using "$dir_data/${country}-eusilc_validation_sample.dta", clear
...
forval year = `min_year'/`max_year' { 
	...
	twoway__histogram_gen valid_yGrossBuLevelYear if ///
		year == `year', width(750) den gen(d_valid v2)
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg ind_gross_inc_`year'_all ///
		...
		rows(2) ///
		graphregion(color(white)) ///
		note("Notes: Amounts in Euro per year, 2015 prices. Sample include males age 18-65. Top and bottom percentiles trimmed. Gross income is" "the sum of capital income, private pension income and employment income.", ///
		size(vsmall)) 
...
forval year = `min_year'/`max_year' { 
	...
	* Plot all hours
	...
	foreach ls in $ls_cat {
		...
		* Plot by weekly hours work
		twoway (hist sim_yGrossBuLevelYear if year == `year' & ///
			sim_labHrsWorkEnumWeek == "`ls'", width(750) ///
			color(green%30) legend(label(1 "SimPaths"))) ///
		(hist valid_yGrossBuLevelYear if year == `year' & ///
			valid_labHrsWorkEnumWeek == "`ls'", width(750) color(red%30) ///
			legend(label(2 "SILC"))) , ///
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg ind_gross_inc_`year'_all ///
		...
		rows(2) ///
		graphregion(color(white)) ///
		note("Notes: Amounts in Euro per year, 2015 prices. Sample include females age 18-65. Top and bottom percentiles trimmed. Gross income is" "the sum of capital income, private pension income and employment income.", ///
		size(vsmall)) 
...
use "$dir_data/SILC_validation_full_sample.dta", clear
