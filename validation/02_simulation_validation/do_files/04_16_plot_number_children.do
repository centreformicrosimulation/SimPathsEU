...
* COUNTRY: 			PL
********************************************************************************
* NOTES: 			This do file plots simulated and SILC % of benefit units
...
(line valid_children_0 year, sort color(green) ///
	legend(label(2 "No children, SILC"))) ///
...
(line valid_children_2 year, sort color(red) ///
	legend(label(6 "2 children, SILC"))) ///
...
(line valid_children_3p year, sort color(grey) ///
	legend(label(8 "3+ children, SILC"))), ///
...
* Prepare validation data
use year valid_new_born dwt demMaleFlag demAge using ///
	"$dir_data/${country}-eusilc_validation_sample.dta", clear

keep if demMaleFlag == 0 
keep if inrange(demAge,18,40)	
	
* Calculate share by year 
collapse (mean) valid_new_born [aw = dwt], by(year)

save "$dir_data/temp_valid_stats.dta", replace
	
	
* Prepare simulated data
use run year sim_new_born demMaleFlag demAge using ///
	"$dir_data/simulation_sample.dta", clear

keep if inrange(demAge,18,40) 
keep if demMaleFlag == 0	
	
collapse (mean) sim_new_born, by(run year)

collapse (mean) sim_new_born ///
		 (sd) sim_new_born_sd = sim_new_born ///
		 , by(year)
		 
foreach varname in sim_new_born  {
...
twoway (rarea sim_new_born_h sim_new_born_l year, ///
	sort color(green%20) legend(label(1 "Simulated"))) ///
(line valid_new_born year, sort color(green) ///
	legend(label(2 "SILC"))), ///
	title("Share of Females with Who Have New Born Baby") ///
	subtitle("Age 18-40") /// 
	xtitle("Year", size(small)) ///
	ytitle("Share", size(small)) ///
	graphregion(color(white)) ///
...
* Save figure
graph export "$dir_output_files/children/validation_${country}_birth_ts_18_40_female.jpg", ///
	replace width(2400) height(1350) quality(100)
