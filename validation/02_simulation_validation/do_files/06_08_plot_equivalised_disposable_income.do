...
(line valid_y_eq_disp_bu_yr year, sort color(green) ///
	...
	note("Notes: Equivalised disposable income assigned to each person computed by adjusting benefit unit's disposable income by the" "modified OECD scale. Samples includes all individuals ages ${min_age}-${max_age}. Trimmed sample. Amounts in 2015 prices.", ///
	size(vsmall))
...
* Prepare validation data
use year dwt valid_y_eq_disp_bu_yr dgn using ///
	"$dir_data/${country}-eusilc_validation_sample_long.dta", clear
...
(line valid_y_eq_disp_bu_yr year, sort color(green) ///
	...
	graphregion(color(white)) ///
	note("Notes: Equivalised disposable income assigned to each person computed by adjusting benefit unit's disposable income by the" "modified OECD scale. Samples includes males ages 18-65. Trimmed sample_long. Amounts in 2015 prices.", ///
	size(vsmall))
...
(line valid_y_eq_disp_bu_yr year, sort color(green) ///
	...
	ylabel(,labsize(small)) xlabel(,labsize(small)) ///
	graphregion(color(white)) ///
	note("Notes: Equivalised disposable income assigned to each person computed by adjusting benefit unit's disposable income by the" "modified OECD scale. Samples includes females ages 18-65. Trimmed sample_long. Amounts in 2015 prices.", ///
	size(vsmall))
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg eqdisp_inc_`year'_all eqdisp_inc_`year'_ZERO ///
		...
		graphregion(color(white)) ///		
		note("Notes: Distribution of benefit unit equivalised disposable, individual. Individual observations plotted, ${min_age}-${max_age} yo. Values in € per year," "2015 prices. Top and bottom percentiles trimmed. Weekly hours worked categories: ZERO = 0, TWENTY = [1,39], FORTY = 40, FIFTY = 41+", ///
		size(vsmall)) 
...
* Prepare validation data
use year dwt valid_y_eq_disp_bu_yr laboursupplyweekly_hu dgn using ///
	"$dir_data/${country}-eusilc_validation_sample_long.dta", clear
...
forval year = `min_year'/`max_year' {
	...
	* Plot all hours
	twoway (hist equivalisedincome if year == `year' , ///
		color(green%30) legend(label(1 "Simulated"))) ///
	(hist valid_y_eq_disp_bu_yr if year == `year' , color(red%30) ///
		legend(label(2 "Observed"))) , ///
