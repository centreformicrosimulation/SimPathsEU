...
********************************************************************************
********************************************************************************
* 1.1 : Mean values over time - By individual, benefit unit amount
********************************************************************************
...
(line valid_y_gross_labour_bu_yr year, sort color(green) ///
	...
	note("Note: Amount at the benefit unit level, individual data plotted. Statistics calculated on the sample of employed individuals" "ages ${min_age}-${max_age}. 2019 X-sectional data used in underlying estimation. Amounts in 2015 prices. Top and bottom percentiles trimmed.", ///
	size(vsmall))
...
* Prepare validation data
use year dwt les_c4 valid_y_gross_labour_bu_yr dgn using ///
	"$dir_data/${country}-eusilc_validation_sample_long.dta", clear
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg gross_labour_inc_`year'_all  ///
		...
		graphregion(color(white)) ///
		note("Notes: Amount at the benefit unit level, individual data plotted. Individuals aged ${min_age}-${max_age}. Values in € per year, 2015 prices. Weekly hours worked categories:" "ZERO = 0, TWENTY = [1,39], FORTY = 40, FIFTY = 41+. Top and bottom percentiles trimmed.", ///
		size(vsmall)) 
