...
foreach varname in hours_cat_1 hours_cat_2 hours_cat_3  ///
	{
...
collapse (mean) hours_cat_* ///
		 (sd) hours_cat_1_sd = hours_cat_1 ///
		 hours_cat_2_sd = hours_cat_2 ///
		 hours_cat_3_sd = hours_cat_3, by(year)
...
foreach y in `years' {
	...
	twoway (bar prop_SILC x_SILC if year == `y', barw(0.4) color(red%50)) ///  
	       ...
	       (rcap hours_cat__high hours_cat__low x_sim if year == `y', lcolor(green)), ///
			...
			legend(size(small)) ///	
	        graphregion(color(white)) ///
			note("Notes: Ages 16-65. ", size(vsmall))
...
collapse (mean) hours_cat_* ///
		 (sd) hours_cat_1_sd = hours_cat_1 ///
		 hours_cat_2_sd = hours_cat_2 ///
		 hours_cat_3_sd = hours_cat_3, by(year demMaleFlag)
...
foreach y in `years' {
	foreach s in `sexes' {
		...
		* Get the label text 
		...
		twoway (bar prop_SILC x_SILC if year == `y' & demMaleFlag == `s', ///
			...
			legend(size(small)) ///	
			graphregion(color(white)) ///
			note("Notes: Comparison for `sextext' in `y'.", size(vsmall))
...
twoway (line valid_hours year, sort color(cranberry) ///
	legend(label(1 "SILC, continuous"))) ///
...
//	legend(label(2 "SILC, discretized"))) ///
