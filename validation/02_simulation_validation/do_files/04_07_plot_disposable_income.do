...
* COUNTRY: 			PL
********************************************************************************
* NOTES: 			This do file plots simulated and SILC disposable income, 
...
(line valid_yDispBuLevelYear year, sort color(green) ///
	...
	note("Notes: Amounts in Euro per year, 2015 prices. Top and bottom percentiles trimmed.", ///
	size(vsmall))
...
forval year = `min_year'/`max_year' {
	...
	twoway (hist sim_yDispBuLevelYear if year == `year', width(750) ///
		color(green%30) legend(label(1 "SimPaths"))) ///
	(hist valid_yDispBuLevelYear if year == `year', ///
		 width(750) color(red%30) legend(label(2 "SILC"))) , ///
		title("Benefit Unit Disposable Income") ///
		subtitle("`year'") ///
...
forval year = `min_year'/`max_year' {
    ...
    * Prepare info needed for dynamic y axis labels 
	twoway__histogram_gen sim_yDispBuLevelYear if year == `year', width(750) ///
		den gen(d_sim v1)
	...
	* Plot all hours
	twoway (hist sim_yDispBuLevelYear if year == `year', width(750) ///
		color(green%30) legend(label(1 "SimPaths"))) ///
	(hist valid_yDispBuLevelYear if year == `year', width(750) ///
		color(red%30) ///
		legend(label(2 "SILC"))) , ///
		subtitle("ALL hours") ///
		name(disp_inc_`year'_all, replace) ///
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg disp_inc_`year'_all ///
		...
		subtitle("`year'") ///		
		legendfrom(disp_inc_`year'_ZERO) ///
		rows(2) ///
		graphregion(color(white)) ///		
		note("Notes: Amounts in Euro per year, 2015 prices. Indiviudal level data of benefit level amount plotted." "Top and bottom percentiles trimmed.", ///
		size(vsmall)) 
		
	graph export ///
	"$dir_output_files/income/disposable_income/validation_${country}_disposable_income_bu_dist_`year'_hrs_work.png", ///
