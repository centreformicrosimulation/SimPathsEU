...
********************************************************************************
********************************************************************************
* 1.1 : Mean values over time, Working age, All
********************************************************************************
...
(line dlltsd year, sort color(green) ///
	...
	note("Notes:Sample contains those aged ${min_age}-${max_age}.", size(vsmall)) 
...
********************************************************************************
* 1.2 : Mean values over time, Working age, By gender
********************************************************************************	
* Prepare validation data 
...
(line dlltsd year if dgn == 1, sort color(red) ///
	legend(label(4 "Male, SILC"))), ///
title("Disabled/Long-term sick by gender ") 	xtitle("", size(small)) ///
	ytitle("Share", size(small)) ///
	legend(size(small)) ///
	xlabel(, labsize(small)) ///
	ylabel(, labsize(small)) ///
...
********************************************************************************
* 1.2 : Mean values over time, Working age, By age
********************************************************************************

* Prepare validation data 
use year dwt dlltsd dgn ageGroup using ///
	"$dir_data/${country}-eusilc_validation_sample_long.dta", clear
...
foreach vble in "dlltsd" {
	...
	twoway (rarea `vble'_1_sim_high `vble'_1_sim_low year, ///
		...
		(line `vble'_1_valid year, sort legend(label(2 "SILC"))), ///
		title("age 15-19") name(`vble'_1, replace) ylabel(0 [0.05] 0.1) ///
	graphregion(color(white))
	
	twoway (rarea `vble'_2_sim_high `vble'_2_sim_low year, ///
		sort color(red%20) legend(label(1 "Simulated") position(6) rows(1))) ///
		(line `vble'_2_valid year, sort legend(label(2 "SILC"))), ///
		title("age 20-24") name(`vble'_2, replace) ylabel(0 [0.05] 0.1) ///
	graphregion(color(white))
	
	twoway (rarea `vble'_3_sim_high `vble'_3_sim_low year, ///
		sort color(red%20) legend(label(1 "Simulated") position(6) rows(1))) ///
		(line `vble'_3_valid year, sort legend(label(2 "SILC"))), ///
		title("age 25-29") name(`vble'_3, replace) ylabel(0 [0.05] 0.1) ///
	graphregion(color(white))
	
	twoway (rarea `vble'_4_sim_high `vble'_4_sim_low year, ///
		sort color(red%20) legend(label(1 "Simulated") position(6) rows(1))) ///
		(line `vble'_4_valid year, sort legend(label(2 "SILC"))), ///
		title("age 30-34") name(`vble'_4, replace) ylabel(0 [0.05] 0.1) ///
	graphregion(color(white))
	
	twoway (rarea `vble'_5_sim_high `vble'_5_sim_low year, ///
		sort color(red%20) legend(label(1 "Simulated") position(6) rows(1))) ///
		(line `vble'_5_valid year, sort legend(label(2 "SILC"))), ///
		title("age 35-39") name(`vble'_5, replace) ylabel(0 [0.05] 0.1) ///
	graphregion(color(white)) 
	...
	twoway (rarea `vble'_6_sim_high `vble'_6_sim_low year, ///
		...
		legend(label(2 "SILC"))), ///
		title("age 40-59") name(`vble'_6, replace) ylabel(0 [0.05] 0.1) ///
	graphregion(color(white))
	
	twoway (rarea `vble'_7_sim_high `vble'_7_sim_low year, ///
		sort color(red%20) legend(label(1 "Simulated") position(6) rows(1))) ///
		(line `vble'_7_valid year, sort legend(label(2 "SILC"))), ///
		title("age 60-65") name(`vble'_7, replace) ylabel(0 [0.05] 0.1) ///
	graphregion(color(white))
	
}

grc1leg dlltsd_1 dlltsd_2 dlltsd_3 dlltsd_4 dlltsd_5 dlltsd_6 ///
	dlltsd_7 , title("Disabled/Long-term sick by age") legendfrom(dlltsd_1) ///
	graphregion(color(white)) ///
