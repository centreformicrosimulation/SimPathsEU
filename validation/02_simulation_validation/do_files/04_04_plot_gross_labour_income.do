...
forval year = `min_year'/`max_year' {
    ...
    * Plot all hours
	twoway (hist sim_yEmpBuGrossLevelYear if year == `year' , width(750) ///
		color(green%30) legend(label(1 "SimPaths"))) ///
	(hist valid_yEmpBuGrossLevelYear if year == `year' , width(750) ///
	...
	* Plot by weekly hours work
	foreach ls in $ls_cat_labour {
		...
		* Prepare info needed for dynamic y axis labels 	
		twoway__histogram_gen sim_yEmpBuGrossLevelYear if year == `year' & ///
			sim_labHrsWorkEnumWeek == "`ls'", width(750) den gen(d_sim v1)
...
forvalues year = 2011/2023 {
	...
	grc1leg gross_lab_inc_`year'_all ///  
		...
		gross_lab_inc_`year'_FORTY  ///
		gross_lab_inc_`year'_FIFTY, ///
		title("Benefit Unit Gross Labour Income by Weekly Hours of Work") ///
		subtitle("`year'") ///
		legendfrom(gross_lab_inc_`year'_TWENTY) ///
		rows(2) ///
		graphregion(color(white)) ///
		note("Notes: Amounts in Euro per year, 2015 prices. Top and bottom percentiles trimmed. Individual observations of benefit unit amount plotted",  ///
		size(vsmall)) 
		
	graph export "$dir_output_files/income/gross_labour_income/validation_${country}_gross_labour_income_bu_dist_`year'_18_65.png", ///
...
forval year = `min_year'/`max_year' {
    ...
    * Plot all hours
	twoway (hist sim_yEmpPersGrossLevelYear if year == `year' , width(750) ///
		color(green%30) legend(label(1 "SimPaths"))) ///
	(hist valid_yEmpPersGrossLevelYear if year == `year' , width(750) ///
		color(red%30) legend(label(2 "SILC"))) , ///
		subtitle("ALL hours") ///
		name(gross_lab_inc_`year'_all, replace) ///
		xtitle("Euro", size(small)) ///
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg gross_lab_inc_`year'_all  ///
		...
		rows(2) ///
		graphregion(color(white)) ///
		note("Notes: Amounts in Euro per year, 2015 prices. Employed 18-65 years olds included in the sample. Top and bottom percentiles trimmed.",  ///
		size(vsmall)) 
		
	graph export "$dir_output_files/income/gross_labour_income/validation_${country}_ind_gross_labour_income_dist_`year'_both.png", ///
		replace width(2400) height(1350) 
