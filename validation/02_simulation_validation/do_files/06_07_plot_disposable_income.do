...
title("Disposable Income") ///
	...
	note("Notes: Statistics computed at the benefit unit level. Plotted using individual level data. Sample includes all individuals ages ${min_age}-${max_age}." "Amounts in 2015 prices. Top and bottom percentials trimmed. ", ///
	size(vsmall))
...
********************************************************************************
********************************************************************************
* 2.1 : Histograms,  By year, and  category of weekly labour supply 
********************************************************************************

* Prepare validation data
use year dwt valid_y_disp_bu_yr laboursupplyweekly_hu dag  ///
	using "$dir_data/${country}-eusilc_validation_sample_long.dta", clear
...
* Prepare simulated data
use run year sim_y_disp_yr_bu laboursupplyweekly dag using ///
...
forvalues year = `min_year'/`max_year' {
	...
	grc1leg disp_inc_`year'_all disp_inc_`year'_ZERO disp_inc_`year'_TWENTY ///
		...
		graphregion(color(white)) ///		
		note("Notes: Sample includes all males aged ${min_age}-${max_age}. Values in € per year (2015 prices). Indiviudal level data for benefit level variable. Samples" "trimmed. Weekly hours worked categories: ZERO = 0, TWENTY = [1,39], FORTY = 40, FIFTY = 41+", ///
		size(vsmall)) 
...
* Prepare validation data
use year dwt valid_y_disp_bu_yr laboursupplyweekly_hu dag dgn ///
	using "$dir_data/${country}-eusilc_validation_sample_long.dta", clear
