...
assigned one of the roles in the HH.
	...
	under the age to leave home (18) will still leave together and set up a new 
	HH. 
	...
	otherwise with father. If neither exists, they will be considered as 
	orphans. 
...
gen n_child = n_child_mother+n_child_father 
	//n of kids this individual has ==> no double count because father's kids 
	//will be in their line while mothers kids will be in their line 
...
count if child == 1 & n_child > 0 //42 obs who are kids but have their own kids 
replace child = 0 if n_child > 0 //rule out   

gen adult = 1 - child 
sum child adult //81% adults
...
fre num_adults //8 obs have no adults 

fre swv idhh if num_adults == 0 
 tab dag if num_adults == 0 //==> these are 16/17 yo kids living alone 
...
replace idbupartner = . if (adult == 1 & idperson != idbenefitunit & ///
	...
	//remove partner id for those who are not head or partner 

replace idbenefitunit = . if (adult == 1 & idperson != idbenefitunit & ///
	idpartner != idbenefitunit) 
	//remove other adults who are not head or partner 
...
replace idbenefitunit = . if (child == 1 & idfather != idbenefitunit & ///
	idmother != idbenefitunit & ((idfather != idbupartner & ///
	idmother != idbupartner) & idbupartner > 0)) 
	//remove kids that are not head's or partner's

* Create new ben unit for single adults in hh 
replace idbenefitunit = idperson if (missing(idbenefitunit) & adult == 1 & ///
	(missing(idpartner) | idpartner < 0)) 

// Loop over number of extra couples residing in the hh that constitute an 
// additional benefit unit 
forvalues i = 1/3 {
	
	//count if adult == 1 & idbenefitunit == . & partnered == 1  
	//8 partnered adults still have no benefit unit
	//list idperson if adult == 1 & idbenefitunit == . & partnered == 1
	
	gsort swv idhh -partnered idbenefitunit -dag idperson 
		//sort so those with a partner who have been assigned a ben unit come  
		//before those who haven't

	
	bys swv idhh: replace idbenefitunit = idperson if ///
	(missing(idbenefitunit) & adult == 1 & !missing(idbenefitunit[_n-1]) & ///
	idpartner != idbenefitunit[_n-1]) & idbupartner[_n-1] != . 
		//added the final condition to ensure only one additional benefit unit 
		//is created 

	//ensure the partner is assigned to the same benefit unit if don't align 
	//by age
	...
	swv == swv[_n-1] & idbenefitunit == .  // fill down idbu for household 
	
	replace idbenefitunit = . if child == 1 & partnered == 0 & ///
	idbenefitunit != idmother & idbenefitunit != idfather 
		//keep ben unit info for children of the first benefit unit 
 
	replace idbenefitunit = . if idbenefitunit != idpartner & ///
	idbupartner == . & partnered == 1 
		//remove if this individual is in a partnership and the id doesn't
		//correspond to their partner 
	
	replace idbupartner = idperson if idbenefitunit != . & ///
	idbupartner == . & partnered == 1 
		//the person who caught the last condition is the benefit units partner 
...
** Assign remaining children to benefit unit 
...
* If some kids are still not assigned - assign them to father's ben units 
forvalues i = 1/20 {
...
count if child == 1 & idbenefitunit == . //864 kids are still not assigned 

* Address orphans 
*bys swv idhh: replace idbenefitunit = idbenefitunit[1] if ///
*	missing(idbenefitunit) & orphan ==1  
	//for now assign orphans to the first benunit 
cap drop orphan 
gen orphan = (idfather < 0 & idmother < 0 & child == 1)
fre orphan if idbenefitunit == . //864 => all remaining are orphans 

cap drop n_orphan
bys stm idhh: egen n_orphan = sum(orphan)  
	//count N of orphans per household ==> can be up to 10!
fre n_orphan	
/*
n_orphan
...
--------------+--------------------------------------------
Valid   0     |     508512      99.51      99.51      99.51
        1     |       1906       0.37       0.37      99.88
        2     |        352       0.07       0.07      99.95
        3     |        107       0.02       0.02      99.97
        4     |         48       0.01       0.01      99.98
        5     |         56       0.01       0.01      99.99
        6     |          9       0.00       0.00     100.00
        7     |         10       0.00       0.00     100.00
        10    |         12       0.00       0.00     100.00
        Total |     511012     100.00     100.00           
-----------------------------------------------------------
...
// Loop over each possible orphan and create corresponding age variables
forvalues i = 1/10 {  
...
// Loop over the number of orphans in a hh 
...
* Assign orphans to their new mothers' benunits (they are heads or partners) 
...
replace idbupartner = temp_idbupartner if orphan == 1 
	//fill in benefit unit partner id 
...
** Run checks 
* Check if all kids are assigned 
count if idbenefitunit == . // 1
count if child == 1 & idbenefitunit == . //2 obs are still not assigned 
count if child == 1 & idbenefitunit == . & orphan == 1

drop if child == 1 & idbenefitunit == . & orphan == 1
...
bys swv idbenefitunit (idbupartner): replace idbupartner = idbupartner[1] if ///
	idbupartner != idbupartner[1] 
	//(8,024 real changes made) this applies to children missing other parents 
	//information 
...
duplicates report stm idperson //no such cases 
...
save "$dir_data/${country}-eusilc_validation_sample_prep2.dta", replace  
